...
Order = Struct.new(:id, :order_lines, :shipping_address, :billing_address, :subtotal, :shipping, :sales_tax, :total, keyword_init: true) do
  def initialize(*args, **attributes, &block)
    ...
    self.shipping_address = Address.new(shipping_address_attributes) # TODO instantiate Address model while passing in attributes
    ...
    self.billing_address = Address.new(billing_address_attributes) # TODO instantiate Address model while passing in attributes
  end
end
